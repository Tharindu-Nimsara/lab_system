...
- **2026-07-20** — Bill printing: the patient copy and the lab worksheet copy can now be printed separately. `BillPdfService.render` takes a `Copy` (PATIENT / WORKSHEET / BOTH); `GET /invoices/{id}/pdf?copy=patient|worksheet|both` (default both). POS shows three buttons — Print patient copy, Print lab copy, Print both. Verified all three variants live (patient/worksheet = 1 page each, both = 2 pages) with distinct filenames. (Note: `ReportEmailConsentTest` errors locally on JDK 25 Mockito self-attach — needs `-Djdk.attach.allowAttachSelf=true`; unrelated to app code, passes in CI.)
